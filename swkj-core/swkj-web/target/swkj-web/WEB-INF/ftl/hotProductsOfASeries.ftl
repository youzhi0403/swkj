...
<div class="wrapper container">
    <h2 class="brandTitle"><p><#if series.s_name??>${series.s_name}<#else></#if></p></h2>
    <div class="description"></div>
    <div class="clearfix"></div>
    <ul class="products row">

    <#--<#if series.page_url?? >${series.page_url}<#else></#if>-->
    <#if list??>
        <#list list as product>
            <li class="col-sm-3 col-xs-6">
                <a href="${contextPath}<#if product.page_url?? >${product.page_url}<#else></#if>"
                   title="<#if product.product_name?? >${product.product_name}<#else></#if>">
                    <div class="product-item">
                        <div class="item"
                             style="background-image:url(<#if product.picture_url?? >${product.picture_url}<#else></#if>)">
                            <div class="title"><#if product.product_name?? >${product.product_name}<#else></#if></div>
                        </div>
                    </div>
                </a>
            </li>
