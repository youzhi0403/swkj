...
<div class="wrapper container">
    <div class="row product-menu">
        ...
        <!--
         ...
         2.a标签的title属性要改
         3.h5标签的内容
         4.style里的background-image:url标签要改
        -->

    <#if list??>
        <#list list as news>
            <div class="col-sm-4 product-menu-item">
                <a href="${contextPath}<#if news.page_url?? >${news.page_url}<#else></#if>"
                   title="<#if news.title?? >${news.title}<#else></#if>">
                    <div class="product-menu-image-container"
                         style=" background-image:url(<#if news.title_picture_url?? >${news.title_picture_url}<#else></#if>)"></div>
                    <h5 class="product-menu-title"><#if news.title?? >${news.title}<#else></#if></h5>
                </a>
            </div>
        </#list>
    <#else >
